...
def generate_cover_letter(resume_text, job_description):
    ...
    - If a similar template exists → reuse structure
    - Otherwise → generate from scratch using LLM
    """
    api_key = os.getenv("GOOGLE_API")
    if not api_key:
    ...
    if not resume_text.strip():
        raise ValueError("Empty resume input")
    ...
    if similar_template:
        logger.info(f"Using existing template (Similarity: {similar_template['similarity']:.2f})")
        prompt = PromptTemplate(
            input_variables=["template", "resume", "job_desc"],
            template="""
        Use this TEMPLATE format to generate a new cover letter:
        ...
        Write a professional and fully personalized cover letter.
        """
        )
        ...
        logger.info("No existing template found, generating from scratch.")
        prompt = PromptTemplate(
            input_variables=["resume", "job_desc"],
            template="""
        Using this resume:
        ...
        Write a professional, compelling cover letter tailored to this position.
        """
        )
        final_prompt = prompt.format(resume=resume_text, job_desc=job_description)
    ...
    if not similar_template:
        store_job_application(job_description, cover_letter)
        logger.info("New template stored for future reuse.")
